...
import { SceneDetectionData } from '@ph-app/models/ph-phash-app-ui/phash.model'

@Component({
  selector: 'app-ph-scene-detection-black-chart',
  templateUrl: './ph-scene-detection-black-chart.component.html',
  styleUrls: ['./ph-scene-detection-black-chart.component.scss']
})
export class PhSceneDetectionBlackChartComponent implements OnInit {
  ...
  selectVideo(_, $) {
    this.playVideo$.emit($)
    for (let x of this.bChartData["timecode_of_scenes_between_black_frames"]) {
  ...
  _triggerGraph(_: SceneDetectionData[]) {
    this._createSVG()
    ...
    var c = []
    _['timecode_of_scenes_between_black_frames'].forEach(data => {
    ...
    var mousemove = function (d, $) {
      var a, b
      for (let x of _["timecode_of_scenes_between_black_frames"]) {
